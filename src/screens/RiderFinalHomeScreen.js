...
import { syncDeliveryStatus } from "../helpers/syncDeliveryStatus"
...
const RiderFinalHomeScreen = ({ navigation, route }) => {
  ...
  // Add this state to track the unsubscribe function
  const [syncUnsubscribe, setSyncUnsubscribe] = useState(null)
        ...
        async (snapshot) => {
          ...
          const availableOrders = snapshot.docs
            .map((doc) => {
              ...
              const isValidTime =
                pickupTime.getHours() > now.getHours() ||
            ...
            .filter(
              (order) =>
                !declinedOrders.includes(order.id) && (order.isPickupValid || (!order.pickupDate && !order.pickupTime)),
            )
  ...
  // Add a cleanup effect for the sync function
  useEffect(() => {
    return () => {
      // Clean up the sync listener when component unmounts
      if (syncUnsubscribe) {
        syncUnsubscribe()
        console.log("Cleaned up delivery status sync listener")
      }
    }
  }, [syncUnsubscribe])
  ...
  const onDeliveryDone = async () => {
    ...
    try {
      console.log("Delivery completed wala try main")
      ...
      console.log(`Started delivery status sync for completed order in onDeliveryDone: ${order.id}`);

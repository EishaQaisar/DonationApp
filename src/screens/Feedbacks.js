...
// const { donationData, item } = route.params || {}; // Ensure fallback to avoid crash
...
const StarRating = ({ rating, setRating, maxStars = 5 }) => {
  ...
  return (
    <View style={styles.ratingContainer}>
      {/* Star rating component code remains the same */}
...
const getCategoryColors = (id) => {
  // Category colors code remains the same
  switch(id) {
    case 'food': return {
...
const CategorySelector = ({ selectedCategory, setSelectedCategory }) => {
  // Category selector code remains the same
  const categories = [
    { 
      id: 'food', 
      label: t('Culinary Experience'),
      description: 'Quality, taste, and presentation of food items',
    },
    { 
      id: 'clothing', 
      label: t('Fashion & Apparel'),
      description: 'Quality, fit, and style of clothing products',
    },
    { 
      id: 'education', 
      label: t('Educational Content'),
      description: 'Quality and effectiveness of learning materials',
    },
  ]

  return (
    <View style={styles.categoriesContainer}>
      <Text style={styles.sectionTitle}>{t('Select Category')}</Text>
...
const Feedbacks = ({item}) => {
  // Move the AuthContext inside the component
  ...
  const handleSubmit = async () => {
    if (!feedback.trim()) {
      ...
      Alert.alert(
        ...
        t("Please provide your feedback comments")
      )
      return
    }
    ...
    if (!category) {
      Alert.alert(
        t("Missing Information"), 
        t("Please select a feedback category")
      )
      return
    }
...
; // Ensure fallback to avoid crash

      // Create feedback data object with recipient and donor information
      const feedbackData = {
        userId: userId,    
        //itemName:donationData?.title,                              // Recipient ID (current user)
        recipientName: user?.username || "",             // Recipient's name from AuthContext
        recipientFullName: user?.name || "",             // Recipient's full name if available
        
        donorId: item.donorId || "",            // Donor's ID
        donorName: item.donorName || "",        // Donor's name
        donationId: item.id || "",              // ID of the donation
        donationTitle: item.title || "",        // Title of the donation
      ...
      Alert.alert(
        t("Error"),
        t("There was a problem submitting your feedback. Please try again.")
      );
    }
  }
  ...
  const getCategoryTitle = () => {
    switch(category) {
      case 'food': return "Culinary Feedback";
      case 'clothing': return "Fashion Feedback";
      case 'education': return "Educational Feedback";
  ...
  const getPlaceholder = () => {
    switch(category) {
      case 'food': 
        return "Share your thoughts on taste, presentation, portion size, etc...";
      case 'clothing': 
        return "Tell us about fit, quality, style, comfort, etc...";
      case 'education': 
        return "Comment on content quality, clarity, effectiveness, etc...";
      default: 
          ...
          >
            ...
            {donationData && (
              <View style={styles.donationInfoContainer}>
                <Text style={styles.donationInfoTitle}>
                  Feedback for donation:
                </Text>
                <Text style={styles.donationInfoText}>
                  {donationData.title || "Unnamed Donation"}
                </Text>
                <Text style={styles.donationInfoSubtext}>
                  by {donationData.donorName || "Anonymous Donor"}
                </Text>
              </View>
            )}
            ...
            <View style={styles.feedbackSection}>
              <Text style={[styles.sectionTitle, { color: getCategoryColor() }]}>
                {category ? getCategoryTitle() : t("Your Comments")}
              </Text>
              ...
              <Text style={[styles.charCount, { color: getCharCountColor() }]}>
                {MAX_CHARS - feedback.length} {t("characters remaining")}
              </Text>
            </View>
            ...
            <Text style={styles.privacyNote}>
              {t("Your feedback is confidential and helps us improve our services.")}
            </Text>
          </Animated.View>
            ...
            >
              <View style={styles.successContent}>
                ...
                <Text style={styles.successText}>
                  {t("Your valuable feedback has been submitted successfully. We appreciate your time and input!")}
                </Text>
              </View>
            </Animated.View>
          )}

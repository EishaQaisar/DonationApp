...
export default function ScheduleRDeliveryScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    const fetchClaimerAddress = async () => {
      try {
        ...
        const profileDoc = await firestore().collection("individual_profiles").doc(uid).get()
  ...
  useEffect(() => {
    if (!donorUsername) return // Prevent running when username is empty

    const fetchDonorAddress = async () => {
      try {
        // Step 1: Get the UID of the donorUsername
        const userQuery = await firestore()
          .collection("users") // Collection where user details are stored
          .where("username", "==", donorUsername)
          .get()

        if (userQuery.empty) {
          console.warn("No user found with username:", donorUsername)
          setDonorAddress("") // Clear address if user not found
          return
        }

        const userData = userQuery.docs[0].data()
        const address = userData.address // Extract address
        console.log("Donor's Address:", address)

        setDonorAddress(address) // Update state with fetched address

        // Set the donor's address as the pickup location
        setPickupLocation(address)

        // Get coordinates for the donor's address
        // validateAndGetCoordinates(address, "pickup")
      } catch (error) {
        console.error("Error fetching donor's address:", error)
        setDonorAddress("") // Handle errors gracefully
      }
    }
          ...
          <ScrollView contentContainerStyle={styles.scrollContainer} keyboardShouldPersistTaps="handled">
            {loading ? (
              ...
              <>
                  ...
                  <View style={styles.itemDetailsContainer}>
                    ...
                    {claimedItem.claimerUsername && (
                      <Text style={styles.itemDescription}>
                        {t("scheduleDelivery.claimedBy")}: {claimedItem.claimerUsername}
                      </Text>
                    )}
                    {claimedItem.itemId && (
                      <Text style={styles.itemQuantity}>
                        {t("scheduleDelivery.itemId")}: {claimedItem.itemId}
                      </Text>
                    )}
                  </View>
                )}
                ...
                <View style={[styles.calendarContainer, validationErrors.date && styles.errorBorder]}>
                  <CalendarPicker
                    onDateChange={(date) => {
                ...
                <Button mode="contained" onPress={() => setShowTimePicker(true)} style={styles.button}>
                  {pickupTime
                    ...
                    : t("scheduleDelivery.setPickupTime")}
                </Button>
                ...
                <TextInput
                  style={[styles.input, validationErrors.pickupLocation && styles.errorBorder]}
                ...
                <TextInput
                  style={[styles.input, validationErrors.dropOffLocation && styles.errorBorder]}
